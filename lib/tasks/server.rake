...
desc "Restarts non-rails server"
task :restart_non_rails do
  Rake::Task[:stop_non_rails].invoke
  Rake::Task[:start_non_rails].invoke
end

desc 'Starts non-rails servers'
task :start_non_rails do
  system("screen -dmS mqtt rake wtmqtt:subscribe")
  system("screen -dmS sidekiq bundle exec sidekiq")
  system("screen -dmS scheduler rake scheduler")
end

desc 'Stop non-rails server'
task :stop_non_rails do
  begin
    system("screen -X -S mqtt quit")
  rescue
  end
